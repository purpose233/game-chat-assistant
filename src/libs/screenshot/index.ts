...
import { Bounds, Display, ScreenshotsData } from './interface';
...
export default class Screenshots extends Events {
  ...
  constructor(opts?: ScreenshotsOpts) {
    ...
    this.listenIpc();
    this.$view.webContents.loadURL(SCREENSHOT_WINDOW_WEBPACK_ENTRY);
    if (opts?.lang) {
  ...
  private async capture(display: Display): Promise<string> {
    ...
    try {
      const monitor = Monitor.fromPoint(display.x + display.width / 2, display.y + display.height / 2);
