...
import { Display, ScreenshotsData } from '../../../../libs/screenshot/interface';

import 'react-screenshots/lib/style.css';

export function ScreenshotView(props: { onOk: (buffer: ArrayBuffer, data: ScreenshotsData) => void }) {
  ...
  const onSave = useCallback(
    async (blob: Blob | null, bounds: Bounds) => {
      ...
      window.screenshots.save(buffer, { bounds, display });
    },
    [display]
  );
  ...
  const onOk = useCallback(
    async (blob: Blob | null, bounds: Bounds) => {
      ...
      props.onOk?.(buffer, { bounds, display });
    },
    [display]
  );
